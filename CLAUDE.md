...
**Remaining for backlog #24**: the search-UI phases (2–5 from the original plan — customer-facing description/tags display beyond DoctorCard, the 3-tab search screen itself, predictive debounced search, the search-bar overflow fix) haven't been started. Also worth relaying to the backend team alongside the bug above: the sample Excel template's `Description` column is now required, matching the form. |
