...
| 9 | Appointment status flow: `Upcoming → InSession → Completed`, `Cancelled` | New feature | **Done (client), backend not deployed** | Same `StatusSwipeBar` pattern as #8: `AppointmentStatus` gains `inSession` (between `upcoming` and `completed`) with `.next`/`.advanceLabel`; `EditAppointmentScreen` rewritten to apply reschedule/advance/cancel immediately in place (no more shared Save button). **Reschedule and Cancel verified fully live** against Azure (date-only and status=cancelled PUTs both apply immediately, correct snackbars, correct UI state). **Advance to `inSession` confirmed cleanly blocked** — real `400 validation_error` ("The request field is required") since the backend doesn't recognize the new status value yet; no crash, matches the already-documented ⚠ pending note on endpoint #43. Since the reschedule-only PUT to the same endpoint succeeds, this isolates the gap to the `inSession` enum value specifically, not a general request-shape issue — worth flagging to the backend session that the error message ("field is required") is misleading for an unrecognized enum value, once #43 is updated |
| 10 | Order ID format standardization — `PHSMS-<mmyy>-<seq>` / `PLSMS-<mmyy>-<seq>` / `DASMS-<mmyy>-<seq>` | New feature | **Done (client), backend not deployed** | Turned out mostly backend-only for Pharmacy: `Order.orderNumber` was already an opaque, displayed-verbatim string with zero client-side format assumptions, so the `PHSMS-<mmyy>-<seq>` swap needs **no client change at all** — purely a backend sequence-generator change. Lab-test bookings and appointments never had a human-readable number before, so those two got genuinely new work: added a nullable `bookingNumber`/`appointmentNumber` field to `LabTest`, `AdminLabTestBooking`, `PastAppointment`, `AdminAppointment`, parsed it, and displayed it on every card/detail screen those entities already have (customer Lab Tests list/detail, customer Profile → Appointments, admin Pathology tile/detail, admin Appointments tile/edit screen). **Decided**: new records only, no backfill — same policy as the original Order decision, extended to all three types for consistency. **Built ahead of the backend** — verified live that every touched screen degrades cleanly with the field absent (`null`): no crash, no visible artifact, screens look identical to before this change. Full format spec (`mmyy` + zero-padded per-type monthly-reset `seq`) documented in `docs/API_ENDPOINTS.md` for backend relay |
