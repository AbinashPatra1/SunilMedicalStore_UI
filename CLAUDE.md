...
| 13 | Inventory bulk import (Excel upload and/or barcode scan) | New feature | **Excel import: Done, live end-to-end.** Barcode scanning: not started (deferred pass) | Split into two passes per the user's ask: **Excel import first (done), barcode scanning later**. See "Bulk import (Excel), 2026-09-17" under Admin > Inventory above for the full write-up — client (`BulkImportScreen`, `bulk_import_parser.dart`, `bulk_import_sheet_reader.dart`, new `file_picker`/`excel` deps) and backend (`POST /v1/admin/products/bulk-import`, #78, contract sent ahead of client work so both could build in parallel) both shipped and **verified live against the real Azure backend** — the backend team had #78 deployed matching the documented contract exactly by the time client testing started; a real end-to-end import (2 valid rows, 3 correctly-flagged invalid rows) created one real product and updated another, both confirmed in the Edit form, then the test-created one was deleted to keep the shared backend clean. Hit and documented a real `excel: 4.0.6` package parser bug (chokes on openpyxl's absolute-style worksheet relationship targets) — not worked around since genuine Excel/Sheets/LibreOffice files shouldn't trigger it; flagged in case a real user's file ever does. **Barcode scanning** (deferred, not started): new persisted `Product.barcode` field (built ahead of backend, like `packSize`/`type`), `mobile_scanner` package, two entry points decided — Inventory list app bar (look-up: scan → match existing product by barcode → Edit, or → Add pre-filled if no match) and a scan icon next to the Barcode field on Add/Edit (prefill only) — not yet scoped into a backend contract since it's a later pass. |
