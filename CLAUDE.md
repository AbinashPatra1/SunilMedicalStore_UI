...
**Backend fix confirmed live, 2026-09-23**: the user reported the Admin Doctors list 500 fixed; re-verified on the emulator as admin — `GET /admin/doctors` loads again, and opening Dr. Ananya Sharma shows `Tags` round-tripping ("general physician, fever, body pain"), so `Doctor.tags` now persists server-side too. `Product.tags`/`LabTest.tags` and the admin Lab Test endpoints (#82–86) weren't re-checked this pass.
