  ...
  - **More** (`AdminMoreScreen`, `lib/features/admin/more/`) — a plain
    ...
    - **Delivery Settings** (`/admin/more/delivery-settings`,
      ...
      **delivery fee tier list** (add/remove rows, each an "up to N km → ₹Y"
      ...
      endpoints #71/#72 aren't deployed yet. **Returns section added 2026-09-21** (below a divider): an "Allow returns" switch plus, when on, a "Return window (days)" field (`returnsEnabled`/`returnWindowDays` on #71/#72, **built ahead of the backend**) — see backlog #21.
