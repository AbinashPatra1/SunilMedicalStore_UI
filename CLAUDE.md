...
- Admin area = **parallel** `StatefulShellRoute.indexedStack` with 5 tabs:
  **Inventory** `/admin/inventory` (default), **Appointments**
  `/admin/appointments`, **Orders** `/admin/orders`, **Lab Tests**
  `/admin/lab-tests` (moved here from More, 2026-09-23), **More**
  `/admin/more` (a menu screen, not a direct feature — see below).
  **Discounts moved into More** (`/admin/more/discounts`) the same day. Admin nested routes live under the parent tab, e.g.
  ...
  **More** (`AdminMoreScreen`) is a plain `ListTile` menu — not a feature
  ...
  slot: currently **Notifications**, **Discounts**, **Statistics** (`/admin/more/statistics`, moved here from
...
| 25 | Ops batch: admin nav swap, location prompt/place name, scan beep, returns, cancel rules, status history, admin notifications, "Others" category, auto-refresh lists, "My …" menu names | New feature / UI | **Done — client; backend re-verified live 2026-09-25 (returns, status history, notifications work). Open backend item: refund on an approved return came back `failed`** | **2026-09-23.** (1) Admin bottom nav: **Lab Tests** is now a tab, **Discounts** moved into More (routes `/admin/lab-tests…`, `/admin/more/discounts…`; Discounts lost its sign-out button, Lab Tests gained one). (2) **Location permission is requested right after sign-in** (`requestLocationPermissionIfNeeded()` in `core/location/current_position.dart`, run after the push-permission prompt from `AuthController`) — verified live. (3) **Delivery Settings shows the store's place name**, not just lat/lng: reverse-geocodes the saved coordinates on load (best effort — this emulator has no geocoder, a real device does) and sends/reads an optional `storeAddress` on #71/#72. (4) **Scan beep**: `MainActivity` exposes a `com.sunilmedical.store/beep` method channel (Android `ToneGenerator`, no package) called via `core/utils/scan_beep.dart` on a successful camera scan; also fixed the barcode look-up matching only the *category-filtered* list (an existing product outside the active filter looked "new") — it now fetches the full catalog. Beep itself not audible-verified (emulator). (5) **Returns** (customer picks items + quantities + reason → admin approves/rejects): `OrderStatus` gained `returnRequested`/`returned` (`OrderStatus.fromWire` falls back to `created` for unknown values), `OrderReturn`/`ReturnLine` models, `ReturnOrderScreen` (`/profile/orders/return`), `OrderReturnCard`, admin Approve/Reject on `AdminOrderDetailScreen`; contract #87/#88 (approve refunds only the returned items' value). The Return button shows only for a `delivered` order inside the admin's return window with medicine lines that carry `productId`. **Decided with the user**: request→admin approval, select items, backend-stored notifications. (6) **Cancel rules**: customer can cancel `created`/`processing` (reverses #21's created-only rule; `isCustomerCancellable`), admin `created`/`processing`/`shipped` (`isAdminCancellable`) — admin's Cancel button is now hidden otherwise instead of always shown. (7) **Status history** card block on the admin order detail (`OrderInfoCard.timeline`, from `statusHistory` `[{status, at}]`; falls back to placedOn/deliveredOn until the backend sends it — verified live showing the fallback). (8) **Admin → More → Notifications** (`features/admin/notifications/`, #89–91): list with unread dot, tap → mark read + open the order / lab-test booking / appointment (`NotificationPayload.resolveRoute`; admin lab-test taps now go to the Pathology detail), "Mark all read", refreshed on foreground pushes; degrades to an error + Retry until the backend exists (verified). (9) **`Others` category** added (last) and **Prescription Drugs is first in `ProductCategory.values`**, so every list/grid leads with it; needs backend acceptance of `Others`. (10) **Auto-refresh**: `RefreshOnFocus` (`core/widgets/refresh_on_focus.dart`, `RouteAware`) on every list screen refetches when the screen is pushed or returned to; the bottom-nav shells refetch the tab being switched to. **Gotcha**: a `RouteObserver` can watch only one navigator, so `appRouteObservers` (`core/routes/app_route_observer.dart`) has one per navigator (root + 10 branches) — sharing one crashed with `observer.navigator == null` assertion. `refreshIfIdle` skips a provider already mid-fetch. (11) Profile menu/screens renamed **My Orders / My Appointments / My Lab Tests**. **Never run `dart format lib`** — it reformatted 150+ unrelated files (reverted). `flutter analyze` clean, 20 tests pass. **Not verified live**: Return flow end-to-end and admin approve/reject (endpoints don't exist yet), notifications with data, scan beep audio. **Live re-verification against the fixed backend, 2026-09-25** (the first pass found `GET /admin/orders`, `GET /orders` and `GET /admin/notifications` all 500ing; fixed by the backend and re-run): customer **#87** — Return items on delivered order `PHSMS-0926-0012` (select item, reason, submit) → `status: returnRequested`, in-app "Return requested" push, card "awaiting approval"; admin **#88** — order list shows "Return requested", detail shows the request + Reject/Approve, **Approve → `status: returned`**, customer "Return approved" push, request card flips to "Return approved"; **`statusHistory`** is returned and the admin's status-history block renders all steps with date/time; **Notifications (#89–91)** lists the event, tapping marks it read (`isRead: true`) and opens the order. **Findings, all client-tolerated**: (1) the backend's `returnRequest.items[]` omits `name` — the card now resolves it from the order's own items by `productId`; (2) an old order's `statusHistory` holds *only* the new entries (no backfill), so `orderTimeline` now fills in Created/Delivered from `placedOn`/`deliveredOn` when they're missing; (3) the notification `type` is `return_requested`, not the contract's `order` — `parseNotificationType` maps order/return/appointment/lab-like names to the right screen. **Open backend item**: approving the return set `refundStatus: "failed"` on this Razorpay-paid order (the customer-cancel refund path worked earlier, #20) — worth checking the backend log/Razorpay dashboard; the app shows "Refund failed — contact support". **Not verified**: the *reject* path, `Others` on a product save, `storeAddress` persistence (the emulator has no geocoder so the app sent none), scan beep audio. |
| 26 | Pagination on every growing list — infinite scroll (customer), numbered pages (admin) | New feature | **Done — client; backend paging not deployed yet (contract in `docs/API_ENDPOINTS.md` §Pagination)** | **Decided with the user, 2026-09-26**: infinite scroll for customer lists, numbered page bar for admin lists, **server-side and backwards-compatible** — list endpoints take optional `?page=&pageSize=` (customer 20, admin 15), still return a plain array, and report the total in an `X-Total-Count` header; omitting `page` returns everything as before. **Core** (`lib/core/paging/`): `fetchPageOf` (one page + total → `PageResult`), `PagedState<T>`, `PagedNotifier<T>` (an `AsyncNotifier` base: subclasses supply `fetch(page)` + `keyOf`, and in `build()` watch any filters then `return loadFirst()`, so a filter change restarts at page 1; `loadMore()` appends for infinite lists, `goToPage(n)` replaces for numbered ones; a generation counter drops stale results), and widgets `InfiniteListView` (loads the next page ~400px before the end, also when the first page doesn't fill the screen; spinner / "tap to retry" footer) and `PageNumberBar` (‹ 1 2 3 4 5 ›, or ‹ Page N › when there's no total; hidden for a single page). **Compatibility**: a response with more rows than `pageSize` is treated as "backend ignored paging, this is everything" (no total, so no pager); a later page that repeats loaded rows is dropped and ends the list — verified live that the current backend (which ignores the params) shows everything on one page with no pager and no duplicates. **Lists converted** — customer (infinite): Medicines by category, Search's three tabs (products/lab tests/doctors), My Orders, My Appointments, My Lab Tests, Prescriptions, Lab Tests catalog; admin (numbered, pager in the screen's `bottomNavigationBar` so the FAB sits above it): Inventory, Orders, Prescriptions, Pathology, Appointments, Users, Lab Tests. Left unpaged (small/fixed): banners, promo codes, addresses, payment methods, doctors-this-week, admin doctors, notifications. Each repository gained a `…Page(...)` method next to its old list method (kept for callers that need everything); providers that other code reads as a plain list stay: `prescriptionsProvider` (checkout picker) beside the new `pagedPrescriptionsProvider`, `adminUsersProvider` (appointment "pick user" sheet) beside `adminUsersPagedProvider`. **Admin Inventory**: the "In stock only" switch moved into `AdminInventoryFilters` and `type`/`search`/`inStock` are now sent to `GET /admin/products` too (still also applied client-side), because client-side filtering can't work over a paged list. `refreshIfIdle` now takes any `AsyncValue` provider. `flutter analyze` clean, 34 tests pass (`test/paging_test.dart`: fake-Dio page fetching, unpaged fallback, no-duplicates, numbered pages, both widgets). **Verified live**: admin Orders/Inventory/Users and customer My Orders render correctly against the current (unpaged) backend; real multi-page behaviour is covered by the unit/widget tests until the backend pages. |
