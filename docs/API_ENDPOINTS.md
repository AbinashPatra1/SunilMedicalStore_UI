...
- **Live check 2026-09-22 — fixed and reverified**: the `returnsEnabled`
  persistence bug (saved `true`, read back as `false`) is fixed. Confirmed via
  a full cold app restart after saving, then again from the customer side —
